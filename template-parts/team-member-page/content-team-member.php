<?php
/**
 * Single Team MemberContent
 *
 * @package OXHU
 */

	$team_member_image   = get_field( 'team_member_image' );
	$team_member_quote   = get_field( 'team_member_quote' );
...
<div class="team-member-page__image-container">
<?php if ( ! empty( $team_member_image ) ) : ?>
			<img src='<?php echo esc_url( $team_member_image['sizes']['oxhu_member_image'] ); ?>' alt='<?php echo esc_attr( $team_member_image['alt'] ); ?>' class='team-member-page__image'>
				<?php else : ?>
				<img src='https://res.cloudinary.com/cdn-oxhu/images/v1666690037/empty-team-member/empty-team-member.webp?_i=AA' alt='empty avatar' class='team-member-page__image'>
				<?php endif; ?>

				<?php if ( ! empty( $team_member_quote ) ) : ?>
				<h1 class='team-member-page__quote mobile'>
				<i icon-name='quote'></i>
						<?php echo esc_html( $team_member_quote ); ?>
				</h1>
				<?php endif; ?>
</div>

		<div class='team-member-page__text mobile'>
			<?php
			if ( ! empty( $team_member_name ) ) :
				?>
			<h3 class='team-member-page__name'>
			...
			if ( ! empty( $team_member_title ) ) :
				?>
			<h4 class='team-member-page__title'><?php echo esc_html( $team_member_title ); ?></h4>
				<?php
			endif;
			?>
			...
			if ( ! empty( $team_member_title ) ) :
				?>
			<p class='team-member-page__excerpt'>
